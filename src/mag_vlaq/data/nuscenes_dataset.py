...
from mag_vlaq.data.nuscenes_transforms import base_transform, load_dbimage, path_to_pil_img
...
from mag_vlaq.data.pc_augmentation import PCRandomRotation
from mag_vlaq.models.layers.sparse_utils import batched_coordinates
...
class NuScenesBaseDataset(data.Dataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train"):
        ...
        self.nusc = NuScenes(version=selectversion, dataroot=dataroot, verbose=False)
        ...
        for location in self.location_count:
            _LOG.info(
                "Number of q samples in %s_%s_%s: %d",
                selectversion,
                split,
                location,
                self.location_count[location],
            )
        _LOG.info(
            "Number of q samples in %s_%s_%s: %d",
            selectversion,
        ...
        for selectlocation in selectlocationlist:
            ...
            db_satellite_dir = os.path.join(
                dataroot, f"aerial_{selectversion}_{selectlocation}_{scale}_{zoom}_{size}_satellite"
            )
            db_roadmap_dir = os.path.join(
                dataroot, f"aerial_{selectversion}_{selectlocation}_{scale}_{zoom}_{size}_roadmap"
            )
        ...
        _LOG.info("Number of aerial db in %s: %d", split, len(self.database_infos))
        ...
        _LOG.info("Number of db+q: %d", len(self.database_queries_infos))
    ...
    def __getitem__(self, index):
        if index >= self.database_num:  # query
            ...
            query_image, query_sph, query_bev, query_pc = load_sensordata_from_sampletoken(
                self.nusc, sampletoken, self.args
            )
...
class NuScenesTripletsDataset(NuScenesBaseDataset):
    ...
    def __getitem__(self, index):
        ...
        query_eastnorth = torch.tensor(
            [self.queries_infos[query_index]["east"], self.queries_infos[query_index]["north"]]
        ).float()
        # query_pc = torch.empty(0)
        ...
        negative_db_eastnorth = torch.tensor(
            [[self.database_infos[e]["east"], self.database_infos[e]["north"]] for e in neg_indexes]
        ).float()

...
from mag_vlaq.data.kitti360_transforms import (
    UTONIA_GLOBAL_SCALE,
    UTONIA_GRID_SIZE,
    _causal_modality_blind,
    _dedup_grid,
    _rand_anisotropic_jitter,
    _rand_rotation_outdoor_scene,
    _rand_scale_jitter,
    base_transform,
...
from mag_vlaq.data.kitti360_utils import get_split_locations

_LOG = logging.getLogger(__name__)


def _single_camname(args):
...
class KITTI360BaseDataset(data.Dataset):
    ...
    def __init__(self, args, datasets_folder="datasets", dataset_name="pitts30k", split="train"):
        ...
        selectlocationlist = get_split_locations(split)
        self.queries_infos = []
        self.queries_utms = []
        resize = 320
        for selectlocation in selectlocationlist:
            _LOG.info("Loading KITTI360 location: %s", selectlocation)
            ...
            if split == "train":
                ...
                qimage0203names = qimage0203names[int(len(qimage0203names) * self.args.train_ratio) :]
            _LOG.info("Number of q samples in %s: %d", selectlocation, len(qimage0203names))
            for i_sample, qimage0203name in enumerate(qimage0203names):
                ...
                with open(qposepath) as f:
                    qpose = f.readline().split(" ")
        ...
        _LOG.info("Number of q samples in %s: %d", split, len(self.queries_infos))
    ...
    def __getitem__(self, index):
        ...
        if index >= self.database_num:  # query
            camname = _single_camname(self.args)
            if camname == "00":

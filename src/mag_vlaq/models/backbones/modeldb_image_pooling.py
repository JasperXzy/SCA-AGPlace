...
class GeM(nn.Module):
    ...
    def forward(self, x):
        # x: [b, c, h, w]
        output = nn.functional.avg_pool2d(x.clamp(min=self.eps).pow(self.p), (x.size(-2), x.size(-1))).pow(1.0 / self.p)
...
if __name__ == "__main__":
    ...
    gem = GeM()
    mixvpr = MixVPR(in_channels=c, in_h=h, in_w=w, out_channels=c, mix_depth=4, mlp_ratio=1, out_rows=4)
    convap = ConvAP(in_channels=c, out_channels=c, s1=2, s2=2)
    cosplace = CosPlace(in_dim=c, out_dim=c)
    rrm = RRM(dim=c)
